...
def creation_logic():
    ...
    def process_second_file(df_first):
        # Open a dialog to select the second file, typically the latest contract file for Creation
        second_file_path = filedialog.askopenfilename(title="Select your LATEST Contract File for Creation",
                                                      filetypes=[("Excel files", "*.xlsx;*.xls")])
        if second_file_path:
            ...
            # Find the row index where the desired header ('Creation part #') is located
            header_row_idx = None
    ...
    def process_third_file(df_first):
        # Open a dialog to select the third file, typically the Creation Backlog File
        third_file_path = filedialog.askopenfilename(title="Select your Creation Backlog File",
                                                     filetypes=[("Excel files", "*.xlsx;*.xls")])
        if third_file_path:
    ...
    def process_fourth_file(df_first):
        # Open a dialog to select the fourth file, typically the Creation Sales History File
        fourth_file_path = filedialog.askopenfilename(title="Select your Creation Sales History File",
                                                      filetypes=[("Excel files", "*.xlsx;*.xls")])
        if fourth_file_path:
    ...
    creation_window.configure(bg=bg_color)

    # Add content specific to the Benchmark module
    label = tk.Label(creation_window, text="Welcome Partnership Member", font=("Verdana", 24), bg=bg_color,
                     fg=text_color)
    label.pack(pady=20)
    ...
    instructions = ("Instructions:\n"
                    ...
                    "2. Choose the Latest Contract File for Creation.\n"
                    "3. Select the Creation Backlog File.\n"
                    "4. Pick the Creation Sales History File.\n"
                    "5. Remember to SAVE your final file upon completion.")

    label = tk.Label(creation_window, text=instructions, font=("Verdana", 20), bg=bg_color, fg=text_color)
    label.pack(pady=20)

    # Button to open the file dialog
    open_file_btn = tk.Button(creation_window, text="Select Excel Files", command=open_and_process_file,
